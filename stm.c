...
#if 0
// Errors in the following function are serious, because we may have already made irreversible commits
// to the shared area.   I'll have to think more about how to handle them.  
//
static int write_locked_segment_pages(shared_segment *seg) {
    ...
    for (sl = seg->snapshot_list; sl; sl = sl->next) {
        ...
        if (sl->page_dirty) {
            ...
            if (stm_verbose & 4)
                fprintf(stderr, " %x", page_num);
            
            // If we have modified the page while it was mapped private, we have to put that back into the
            // shared copy.   Either we have to do it by saving a copy of our private page, re-mapping shared,
            // and copying the saved page over the shared page, or we just use pwrite() and have the system worry
            // about it, and only then re-map shared.
            
            if (page_table_elt->current_transaction != seg->transaction_id) {
                if (stm_verbose & 1)
                    fprintf(stderr,
                            "write_locked_segment_pages: page %x belongs to transaction %d instead of %d, last transaction was %d\n",
                            page_num, page_table_elt->current_transaction, seg->transaction_id, 
                            page_table_elt->completed_transaction);
                atomic_spin_lock_unlock(&seg->segment_transaction_data->transaction_lock);
                return -1;
            }
            ...
            // re-use the page snapshot buffer to temporarily keep a copy of the page while we re-map the page as shared,
            // then copy the new contents into it.
            ...
            if (status == (void*)-1) {
                if (stm_verbose & 1)
                    perror("write_locked_pages: mmap failed");
                stm_errno = STM_MMAP_ERROR;
                atomic_spin_lock_unlock(&seg->segment_transaction_data->transaction_lock);
                result = -1;
            }
            ...
            if (pwrite (seg->fd, sl->original_page_va, seg->page_size, 
                        (off_t)(sl->original_page_va - seg->shared_base_va)) == -1) {
                if (stm_verbose & 1)
                    perror("write_locked_pages: pwrite failed");
                stm_errno = STM_WRITE_ERROR;
                atomic_spin_lock_unlock(&seg->segment_transaction_data->transaction_lock);
                result = -1;
            }
            fsync(seg->fd);
    ...
    // re-protect *all* pages with default inter-transaction protection.    Re-map shared.
