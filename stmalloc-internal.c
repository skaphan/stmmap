...
static void set_size_mask(AVLtreeNode *a) {
	stmalloc_node *l, *r;
	stmalloc_node *n = (stmalloc_node *)a;

	n->size_mask = n->size;

	l = (stmalloc_node *)a->left;
	r = (stmalloc_node *)a->right;
	if (l)
...
size_t block_size_for (size_t size) {	
	...
	if (t == 0) t = least_power_of_2_ge(sizeof(stmalloc_node));
...
static void split_node(stmalloc_node* t, size_t size, stmalloc_node **free_list) {
	stmalloc_node *new_node;
	while (t->size > size) {
		t->size >>= 1;
...
//
static AVLtreeNode* stmalloc_search(AVLtreeNode* t, size_t size, stmalloc_node **free_list) {	
	...
	if ((tsize = ((stmalloc_node*)t)->size) == size)
		return t;
	
	size_t left_smallest = t->left? least_power_of_2_gt_in (((stmalloc_node*)t->left)->size_mask, size) : 0;
	size_t right_smallest = t->right? least_power_of_2_gt_in (((stmalloc_node*)t->right)->size_mask, size) : 0;
	...
	if (size > tsize) {
		...
		if (left_smallest == 0) {
			if (right_smallest == 0) {
				...
				result = stmalloc_search(t->right, size, free_list);
			}
		} else {
			if (right_smallest == 0) {
				result = stmalloc_search(t->left, size, free_list);
			} else {
		...
		if (left_smallest && left_smallest < tsize) {
			...
			if (right_smallest && right_smallest < tsize) {
				...
				split_node((stmalloc_node*)t, size, free_list);
			}	
...
void *stm_alloc(size_t size, stmalloc_node **free_list) {	
	
	AVLtreeNode* result;
	size_t real_size;
	
	result = stmalloc_search((AVLtreeNode*)*free_list, block_size_for(size), free_list);
	if (result) {
		real_size = ((stmalloc_node *)result)->size;
...
static void merge_with_buddies(void *base_va, stmalloc_node *freed_object, stmalloc_node **free_list) {
	
	off_t offset, buddy_offset;
	stmalloc_node *buddy_block;
...
//	while (freed_object->size <= MAX_BLOCK_SIZE) {
	...
	while (1) {
		...
		if ((buddy_block = (stmalloc_node *)AVLsearch((AVLtreeNode*)*free_list,
...
static int nodes_overlap_cmp(void* a, void* b) {
	...
	if ((a <= b && b < a + ((stmalloc_node*)a)->size) ||
		(b <= a && a < b + ((stmalloc_node*)b)->size))
		return 0;
	else if (a<b)
		return -1;
	else {
...
void stm_free(void *object_va, size_t size, void *base_va, stmalloc_node **free_list) {
	size_t block_size;
		
	block_size = block_size_for(size);
	
	if (AVLsearch((AVLtreeNode*)*free_list, (AVLtreeNode*)object_va, nodes_overlap_cmp, nodekey) != NULL) {
		fprintf(stderr, "stm_free: node 0x%lx already in free list!\n", (unsigned long)object_va);
		return;
	}
	
	((stmalloc_node*)object_va)->size = block_size;
	AVLaddToTree((AVLtreeNode*)object_va, (AVLtreeNode**)free_list, nodecmp, nodekey);	
	merge_with_buddies(base_va, (stmalloc_node*)object_va, free_list);	
	
}

stmalloc_node **stm_alloc_init(void *base_va, size_t size, int mode) {
	...
	if (mode == 1) {
		...
		stmalloc_node *n;	
		stmalloc_node *tmp_free_list = NULL;

		min_block_size = least_power_of_2_ge(sizeof(stmalloc_node));
		...
		while (remaining_size >= min_block_size) {
			...
			if (first_time) {
				...
				if (stm_alloc(min_block_size, &tmp_free_list) != base_va) {
					fprintf(stderr, "stm_alloc_init:  initial allocation != base_va\n");
					exit(-1);
				}
				*(stmalloc_node**)base_va = tmp_free_list;		// move the tree root to the base of the segment
				first_time = 0;
			} else {
...
static int __verify_tree_integrity(AVLtreeNode *tt, AVLtreeNode* parent, void* lower_bound, void* upper_bound) {
	...
	size_mask = t->size | (tt->right? ((stmalloc_node*)tt->right)->size_mask : 0)
						| (tt->left? ((stmalloc_node*)tt->left)->size_mask : 0);
	...
	if (size_mask != t->size_mask) {
		fprintf(stderr, "Node %lx, size mask is %lx, should be %lx. size=%lx, lmask=%lx, rmask=%lx\n",
				...
				tt->left? ((stmalloc_node*)tt->left)->size_mask:0,
				tt->right? ((stmalloc_node*)tt->right)->size_mask:0);
		result++;
	}
...
int verify_tree_integrity(stmalloc_node *free_list) {
	return __verify_tree_integrity((AVLtreeNode*)free_list, NULL, NULL, NULL);
}

	
	
static void __overlap_check(stmalloc_node *t, void *base, size_t size, void* lower_bound, void* upper_bound) {
	...
	if (base <= (void*)t) {
		if (tt->left)
			__overlap_check((stmalloc_node*)tt->left, base, size, lower_bound, tt);
	
	} 
	if (base >= (void*)t) {
		if (tt->right)
			__overlap_check((stmalloc_node*)tt->right, base, size, (void*)t + t->size, upper_bound);
	}
}
	

void overlap_check(stmalloc_node *t, void *base, size_t size) {
	__overlap_check(t, base, size, NULL, NULL);
}


void print_free_list(stmalloc_node *t) {
	AVLtreeNode *a = (AVLtreeNode *)t;
	
	if (a->left)
		print_free_list((stmalloc_node*)a->left);
	
	printf("[ %lx, %lx ] %lx\n", (unsigned long)t, (unsigned long)t+t->size, (unsigned long)t->size);
	
	if (a->right)
		print_free_list((stmalloc_node*)a->right);
}
